...
public class RenderLiving extends Render
{
    ...
    protected void renderLivingLabel(EntityLiving par1EntityLiving, String par2Str, double par3, double par5, double par7, int par9, int j)
    {
        ...
        if(Vanity.bignames)
        {
	        if(par1EntityLiving.getDistanceToEntity(renderManager.livingPlayer) / 5 <= 1.6)
	        {
	        	f1 = 1.6F;
	        } else
	        if(par1EntityLiving.getDistanceToEntity(renderManager.livingPlayer) / 5 >= 20)
	        {
	        	f1 = 10F;
	        } else
	        {
	        	f1 = par1EntityLiving.getDistanceToEntity(renderManager.livingPlayer) / 5;
	        }
        }
        if(!Vanity.bignames)
        {
        ...
        byte byte0 = -8;
        if (par2Str.equals("xXGravyPopXx"))
        {
            byte0 = -10;
        }
        GL11.glDisable(GL11.GL_TEXTURE_2D);
        ...
        if(Vanity.nameplates)
        {
        	tessellator.setColorRGBA_F(0.0F, 0F, 0.0F, 5F);	
        }else{
        ...
        tessellator.draw();
        GL11.glEnable(GL11.GL_TEXTURE_2D);
    	//fontrenderer.drawStringWithShadow((new StringBuilder()).append("POG Member").toString(), -27, -20, 0xffffff00);
    	//if(Vanity.)
        fontrenderer.drawString(par2Str, -fontrenderer.getStringWidth(par2Str) / 2, byte0, /*Vanity.protectHashMap.containsKey(((EntityPlayer)e).username) ? 39158 :*/ 0xffffffff);
