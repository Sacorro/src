...
public class Tessellator
{
    ...
    public Tessellator(int par1)
    {
        ...
        vertexDatas = null;
        drawnIcons = new boolean[256];
        vertexIconIndex = null;
        vertexCount = 0;
        hasColor = false;
        hasTexture = false;
        hasBrightness = false;
        ...
        vertexIconIndex = new int[bufferSize];
    }
    ...
    private void draw(int i, int j)
    {
        ...
        {
            GL11.glDrawArrays(GL11.GL_TRIANGLES, i, k);
        }
        else
        {
            GL11.glDrawArrays(drawMode, i, k);
        }
    }

    private int drawForIcon(int i, int j)
    {
        GL11.glBindTexture(GL11.GL_TEXTURE_2D, terrainTextures[i]);
        int k = -1;
        int l = -1;

        for (int i1 = j; i1 < addedVertices; i1++)
        {
            int j1 = vertexIconIndex[i1];
    ...
    public int draw()
    {
        ...
        if (renderingChunk && subTessellators.length > 0)
        {
            ...
            for (int k = 0; k < subTessellators.length; k++)
            {
                ...
                Tessellator tessellator = subTessellators[k];
        ...
        if (vertexCount > 0)
        {
            if (Config.isMultiTexture() && renderingChunk && this == instance)
            {
                intBuffer.clear();
                ...
                for (int l = 0; l < addedVertices; l++)
                {
                    int j1 = vertexIconIndex[l];

                    if (!drawnIcons[j1])
                    {
                        l = drawForIcon(j1, l) - 1;
                        i++;
                        drawnIcons[j1] = true;
                    }
                }
    ...
    public void setColorOpaque(int par1, int par2, int par3)
    {
        ...
        if(Vanity.xray)
        {
            setColorRGBA(par1, par2, par3, (int) Vanity.opacity);
        }
    }
    ...
    public void addVertexWithUV(double par1, double par3, double par5, double par7, double par9)
    {
        if (Config.isMultiTexture() && renderingChunk && this == instance)
        {
            ...
            vertexdata.brightness = brightness;
            ...
            addedVertices -= 3;
            double d = (vertexDatas[0].u + vertexDatas[1].u + vertexDatas[2].u + vertexDatas[3].u) / 4D;
            double d1 = (vertexDatas[0].v + vertexDatas[1].v + vertexDatas[2].v + vertexDatas[3].v) / 4D;
            int j;

            if (d > 0.875D && d < 1.0D && d1 > 0.75D && d1 < 0.875D)
            {
                j = 5;
            }
            ...
            int k = (int)(d1 * 16D);
            int l = k * 16 + j;
            double d2 = (double)j / 16D;
            double d3 = (double)k / 16D;
            int i1 = addedVertices;

            for (int j1 = 0; j1 < 4; j1++)
            {
                VertexData vertexdata1 = vertexDatas[j1];
                par1 = vertexdata1.x;
                par3 = vertexdata1.y;
                par5 = vertexdata1.z;
                par7 = vertexdata1.u;
                par9 = vertexdata1.v;
                vertexIconIndex[i1 + j1] = l;
                par7 -= d2;
                par9 -= d3;
                par7 *= 16D;
                par9 *= 16D;
                int k1 = color;
                color = vertexdata1.color;
                int l1 = brightness;
                brightness = vertexdata1.brightness;
    ...
    public void addVertex(double par1, double par3, double par5)
    {
        if (autoGrow && rawBufferIndex >= bufferSize - 32)
        {
            ...
            floatBuffer = byteBuffer.asFloatBuffer();
            shortBuffer = byteBuffer.asShortBuffer();
            int ai1[] = new int[bufferSize];
            System.arraycopy(vertexIconIndex, 0, ai1, 0, vertexIconIndex.length);
            vertexIconIndex = ai1;
        }
